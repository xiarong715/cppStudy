...
local cPlayAudio = {
    ...
    play = function (self, info, cb)    -- info[type = "VOIP", codec = [type = "", rate = "48000", channel = "2", format = "S16LE"]]
      LOGD(TAG, "play in>")
      LOGDTree(info, TAG)
      LOGDTree(DecodeJson(info), TAG)

      info = DecodeJson(info)
      local errmsg
      if not info then errmsg = "info is nil" return LOGD(TAG, errmsg) end
      if not info.type then errmsg = "info.type is nil" return LOGD(TAG, errmsg) end
      if not info.newsocket and not info.uri then errmsg = "info.newsocket and info.uri is nil" return LOGD(TAG, errmsg) end
      if not info.encoding.type then errmsg = "info.encoding.type is nil" return LOGD(TAG, errmsg) end
      if not info.encoding.codec then errmsg = "info.encoding.codec is nil" return LOGD(TAG, errmsg) end
      if not info.encoding.codec.rate then errmsg = "info.encoding.codec.rate is nil" return LOGD(TAG, errmsg) end
      if not info.encoding.codec.channel then errmsg = "info.encoding.codec.channel is nil" return LOGD(TAG, errmsg) end
      if not info.encoding.codec.format then errmsg = "info.encoding.codec.channel is nil" return LOGD(TAG, errmsg) end

      return ipc:sendCall(self.tunnel, ">play", function (pt)
        LOGD("play ipc:sendCall")
        pt:putString(info.type)
        if info.newsocket then pt:putString(info.newsocket) else pt:putString("") end
        if info.uri then pt:putString(info.uri) else pt:putString("") end
        pt:putString(info.encoding.type)
        pt:putString(info.encoding.codec.rate)
        pt:putString(info.encoding.codec.channel)
        pt:putString(info.encoding.codec.format)
        return true
...
function api:serveClient(t, handler)
  ...
  local serving = {
    [">play"] = function (self, msg)
      LOGD(TAG, ">play in>")
      local info = {encoding = {codec = {}}}
      info.type = msg:popString()
      info.newsocket = msg:popString()
      info.uri = msg:popString()
      info.encoding.type = msg:popString()
      info.encoding.codec.rate = msg:popString()
      info.encoding.codec.channel = msg:popString()
      info.encoding.codec.format = msg:popString()
      local callbackId = msg:popString()
